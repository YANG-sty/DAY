...
class HelloActor1 extends Actor{
  override def receive: Receive = {
    ...
    case int: Int => { println("value is "+int)}
    case _=> { println("default.....")}
  }

}

//todo actor通信测试失败，无法正常的初始化 线程池 ActorSystem() Exception in thread "main" java.lang.NoSuchMethodError: scala.Predef$.wrapRefArray([Ljava/lang/Object;)Lscala/collection/mutable/WrappedArray;
object Lesson_Actor{
  // 创建线程池对象myFactoryPool，用来创建actor的对象的
  val myFactoryPool = ActorSystem()
  //通过MyFactory.actorOf方法来创建一个actor，注意，Props方法的第一个参数需要传递我们自定义的HelloActor类，第二个参数是给actor起个名字
  val helloActor: ActorRef = myFactoryPool.actorOf(Props[HelloActor],"helloActor")
